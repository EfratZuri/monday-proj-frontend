<template>
	<section class="grid-row-component task-preview flex align-center">
		<div class="sticky-feature">
			<div
				class="grid-cell-row-component-header name-cell flex align-center floating-cell"
				...
				<div class="conversation-indicator-component">
					...
					>
						...
						<svg
							...
							height="24"
							aria-hidden="true"
							class="icon_component chat-with-update icon_component--no-focus-style"
						>
							<path
						...
						<span v-if="isRecentlyUpdated" class="updates-count" style="left: 13px">{{
...
export default {
	...
	methods: {
		...
		getCmpInfo(col) {
			...
			return { selected: this.task[col.type], data: col.data, groupStyle: this.styleObj };
		},
		getDefaultCmp(col) {
			return { selected: col.data.default, data: col.data, groupStyle: this.styleObj };
		},
		updateTask(ev, type) {
